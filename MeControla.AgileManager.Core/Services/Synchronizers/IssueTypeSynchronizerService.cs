...
namespace MeControla.AgileManager.Core.Services.Synchronizers
{
    public class IssueTypeSynchronizerService : IIssueTypeSynchronizerService
    {
        ...
        {
            var issuetypes = await LoadListFromJira(cancellationToken);

            foreach (var issuetype in issuetypes)
                await SaveIssueType(issuetype, cancellationToken);
        }

        private async Task<IList<IssueTypeDto>> LoadListFromJira(CancellationToken cancellationToken)
            => await issueTypeGetAll.Execute(cancellationToken);
